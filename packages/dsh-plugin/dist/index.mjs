...
function finiteNumber2(...candidates) {
  for (const candidate2 of candidates) {
    const parsed = Number(candidate2);
...
function normalizeUsage(value) {
  ...
  const details = objectOf(value.prompt_tokens_details) ?? objectOf(value.promptTokensDetails) ?? objectOf(value.details);
  ...
  const outputTokens = finiteNumber2(
    value.output_tokens,
    value.outputTokens,
  ...
  const totalTokens = finiteNumber2(value.total_tokens, value.totalTokens, value.totalTokenCount);
  const disjointCacheRead = finiteNumber2(
    value.cache_read_input_tokens,
    value.cacheReadInputTokens,
    value.cachedContentTokenCount
  );
  const includedCacheRead = finiteNumber2(
  ...
  const cacheWriteTokens = finiteNumber2(
  ...
  if (Number.isFinite(inputTokens)) {
    ...
    result.inputTokens = promptIncludesCache ? Math.max(0, inputTokens - cacheRead - cacheWrite) : inputTokens;
  }
